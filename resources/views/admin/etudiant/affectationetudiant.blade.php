...
<div class="card shadow mb-4" style="margin: 40px; padding: 20px;"> 
    <div class="card-header py-3 border-bottom-info text-center">
        <h6 class="m-0 font-weight-bold text-primary">Affectation étudiants-classe</h6>
    </div>
    <div class="card-body">
            ...
            <tbody id="results">
                @forelse ($classes as $classe)
                    <tr>
                        <td>{{ $classe->nom }}</td>
                        <td>
                            <button class="btn btn-primary btn-sm" onclick="affectationetudiantadd('{{ $classe->id }}')">Affecter des étudiants</button>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="2">Aucune classe trouvée</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    function affectationetudiantadd(id) {
        window.location.href = "{{ url('etudiant/affectationetudiantadd') }}/" + id;
    }
</script>
