                ...
                @else
                    @foreach ($seances as $seance)
                        <tr>
                            ...
                            <td>{{$seance->first_date}}</td>
                            <td><button class="btn btn-info btn-circle btn-lg" onclick="adminabsences('{{$affectation->id}}', '{{ $seance->seance }}')"   style="font-size: 0.5em;">Afficher les absences</button></td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
...
<script>
    ...
    function adminabsences(affectation_id, seance) {
        window.location.href = "{{ url('emploi/adminabsences') }}/" + affectation_id + "/" + seance;
    } 
